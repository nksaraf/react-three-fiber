import { createMemo, createRenderEffect, createSignal, JSX, ParentProps, splitProps } from 'solid-js'
import { insert, mergeProps, render } from 'solid-js/web'
...
const useRef = function <T>(initialValue: T) {
  return {
...
export const Html = (props: ParentProps<HtmlProps>) => {
  props = mergeProps(
    {
      eps: 0.001,
      transform: false,
  ...
  const styles = createMemo(() => {
    if (props.transform) {
      ...
      return {
        position: 'absolute' as const,
        transform: props.center ? 'translate3d(-50%,-50%,0)' : ('none' as const),
        ...(props.fullscreen && {
    ...
    if (props.transform && el()) {
      insert(el(), () => (
        <div ref={(el) => (transformOuterRef.current = el)} style={styles()}>
          <div ref={(el) => (transformInnerRef.current = el)} style={{ ...transformInnerStyles(), top: 0, left: 0 }}>
  ...
  useFrame(() => {
    ...
    const vec = props.transform
      ? oldPosition.current!
      : (props.calculatePosition ?? defaultCalculatePosition)(group, camera(), size())
    ...
    if (
      ...
      if (typeof props.occlude === 'boolean') {
        ...
        raytraceTarget = props.occlude.map((item) => item.current) as Object3D[]
      }
      ...
      if (props.transform) {
        ...
        const scale = props.distanceFactor === undefined ? 1 : objectScale(group, camera()) * props.distanceFactor
  ...
  return <T.Group {...groupProps} ref={group} />
}
