...
export function loadFen(fen: string): State | null {
  ...
  for (let i = 0; i < position.length; i++) {
    ...
    if (piece === '/') {
      ...
      const newState = putPiece(state, { type: piece.toLowerCase(), color: color }, algebraic(square))
      if (!newState) {
        return null
      }
      state = newState
...
export function putPiece(prevState: State, piece: { type?: string; color?: string }, square?: string): State | null {
...
export function generateMoves(state: State, options: { legal?: boolean; square?: string } = {}): HexMove[] {
...
export function sanToMove(
  ...
  for (let i = 0, len = moves.length; i < len; i++) {
    ...
    if (cleanMove === strippedSan(san) || (sloppy && cleanMove === strippedSan(moveToSan(state, moves[i], options)))) {
      return moves[i]
    }
    if (
      from &&
