...
import { Color, Comments, GameHistory, HexMove, Move, FenComment, Piece, State, Validation, PartialMove } from './types'
...
export class Chess {
  ...
  public inDraw(): boolean {
    return (
      this._state.half_moves >= 100 || this.inStalemate() || this.insufficientMaterial() || this.inThreefoldRepetition()
    )
  }
   ...
   * ```
   */
  public loadPgn(pgn: string, options: { newline_char?: string; sloppy?: boolean } = {}): boolean {
   ...
   * move notations
   */
  public move(move: string | PartialMove, options: { sloppy?: boolean; dry_run?: boolean } = {}): Move | null {
   ...
   * move notations
   */
  public validateMoves(moves: string[] | PartialMove[], options: { sloppy?: boolean } = {}): Move[] | null {
   ...
   * move notations
   */
  public isPromotion(move: string | PartialMove, options: { sloppy?: boolean } = {}): boolean {
